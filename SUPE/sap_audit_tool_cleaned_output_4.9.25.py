...
Note: Suspicious timing detection is disabled for firefighter IDs who work round the clock.
"""
...
# Import and check versions of required third-party libraries
try:
    import pandas as pd
...
def detect_suspicious_timing(timestamp):
    ...
    Disabled for firefighter IDs who work round the clock.
    """
    # Function is kept for future use but returns False by design
    return False, ""
...
# Full updated header coloring logic for generate_excel_output()
...
def apply_custom_headers(worksheet, df, wb):
    ...
    for i, col in enumerate(df.columns):
        ...
        if "Rationale" in col:
            width = 80
        elif "Msg" in col or "Comment" in col:
            width = 60
        elif col in ("Change_Timestamp", "SM20_Datetime"):
            width = 20
        elif col == "Table_Name":
            width = 25
        elif col == "SysAid#":
...
def generate_excel_output(correlated_df, unmatched_cdpos, unmatched_sm20, output_file):
    ...
    try:
        ...
        with pd.ExcelWriter(output_file, engine="xlsxwriter") as writer:
            ...
            apply_custom_headers(ws_corr, correlated_df, wb)
            ...
            apply_custom_headers(ws_unmatch_cd, unmatched_cdpos, wb)
...
def expanded_risk_tag(row):
    ...
    Analyzes changes to determine risk level based on table sensitivity,
    transaction code, change type, and other factors.
    """
    try:
        # Normalize fields with defensive coding
        ...
        # 4. Check for sensitive transaction codes
        ...
        # 8. Fallback for any other changes
        else:
...
def main():
    ...
    try:
        ...
        try:
            ...
            log_message(f"Error processing risk levels or generating output: {str(e)}", "WARNING")
            return False
    
    except Exception as e:
        log_message(f"Critical error in main execution: {str(e)}", "ERROR")
        import traceback
...
if __name__ == "__main__":
    ...
    log_message(f"Using the following files:")
    log_message(f"SM20 file: {os.path.abspath(SM20_FILE)}")
    log_message(f"CDHDR file: {os.path.abspath(CDHDR_FILE)}")
    log_message(f"CDPOS file: {os.path.abspath(CDPOS_FILE)}")
    log_message(f"Output file: {os.path.abspath(OUTPUT_FILE)}")
    
    # Add a banner
    # Add a banner
    banner = "\n" + "="*80 + "\n"
    banner += " SAP AUDIT TOOL - ENHANCED SECURITY ANALYSIS ".center(80, "*") + "\n"
    banner += " Analyzes SM20, CDHDR, and CDPOS logs for high-risk activities ".center(80) + "\n"
    banner += f" Version {VERSION} ".center(80) + "\n"
    banner += "="*80 + "\n"
    print(banner)
    ...
    try:
        success = main()
        if success:
            log_message("Analysis complete. Results saved to: " + os.path.abspath(OUTPUT_FILE))
        ...
        log_message(f"Fatal error during analysis: {str(e)}", "ERROR")
