...
def expanded_risk_tag(row):
    ...
    (MODIFIED to exclude SysAid# and Comment from Rationale string)
    """
    ...
    # Get additional fields if they exist - KEEP extraction for potential future use or debugging
    ...
    table_key = row.get(CDHDR_OBJECTID_COL, 'N/A') # Extract table key if available
    ...
    if display_but_changed:
        ...
        # if sysaid: # MODIFIED: Commented out SysAid inclusion
        #     details += f" SysAid#: {sysaid}"
        #     # No comment check here in original code for this block
        # details += "." # Add period regardless
    ...
    if is_chg:  # Only assess risk if a change indicator is present
        ...
        if is_sensitive_table:
            ...
            if table in critical_tables:
                ...
                # if sysaid: # MODIFIED: Commented out SysAid inclusion
                #     details += f" SysAid#: {sysaid}."
                # if comment: # MODIFIED: Commented out Comment inclusion
                #     details += f" Comment: {comment[:100]}"
                return "High", details

            # Regular sensitive table change
        ...
        if is_sensitive_tcode:
            ...
            if tcode in direct_db_tcodes and not is_sensitive_table:
                ...
                if field_name and (new_value or old_value):
                    details += f" Field '{field_name}' changed from '{old_value}' to '{new_value}'."
                # No SysAid/Comment check originally here for this specific case
            ...
            # Downgrade monitoring TCodes unless specific message indicates sensitive action
                ...
                if table_key != 'N/A': # Use the extracted table_key variable
        ...
        # Note: Original script had check here `if not is_sensitive_table and not is_sensitive_tcode and not is_direct_db_change and not is_rfc_activity and not debug:`
        # This seems redundant because if those were true, it would have returned earlier.
        # Assuming the intent is "any other change" gets Medium risk.
        details = f"Change (type '{chng}') via TCode '{tcode}' to table '{table}'"
        details += f". (Table/TCode not listed as sensitive, no other High risk factors identified). ChgDoc: {chg_doc_num}."
        if field_name and (new_value or old_value):
             details += f" Field '{field_name}' changed from '{old_value}' to '{new_value}'."
        # if sysaid: # MODIFIED: Commented out SysAid inclusion
        #      details += f" SysAid#: {sysaid}."
        return "Medium", details # Assuming unclassified changes are Medium

    # Default to Low risk if no change indicator or doesn't meet other criteria
    details = f"Activity via TCode '{tcode}' potentially on table '{table}' or system-wide" # Clarified default text
