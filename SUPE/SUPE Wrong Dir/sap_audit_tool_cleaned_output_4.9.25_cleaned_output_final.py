...
def generate_excel_output_cleaned(correlated_df, unmatched_cdpos, unmatched_sm20, output_file):
    import pandas as pd
    import os
    from datetime import datetime

    log_message(f"Generating cleaned Excel output to {output_file}...")

    # Define columns to drop from all sheets
    columns_to_drop_patterns = [
        r"^original_cdpos_index$", r"^original_sm20_index$",
        r"^has_comment$", r"^has_sysaid$", r"^is_display_only$",
        r"^display_but_changed$", r"^has_aging_filter$",
        r"^SysAid#$", r"^comment / review$",
        r".*_x$", r".*_y$"  # Drop all columns with _x or _y suffix from merges
    ]

    def drop_unwanted_columns(df):
        return df[[col for col in df.columns if not any(re.match(p, col) for p in columns_to_drop_patterns)]]
    ...
    try:
        ...
        with pd.ExcelWriter(output_file, engine="xlsxwriter") as writer:
            ...
            # Drop columns before writing
            correlated_cleaned = drop_unwanted_columns(correlated_df)
            correlated_cleaned.to_excel(writer, sheet_name="Correlated_Events", index=False)
            ws_corr = writer.sheets["Correlated_Events"]
            apply_custom_headers(ws_corr, correlated_cleaned, wb)
            ws_corr.autofilter(0, 0, len(correlated_cleaned), len(correlated_cleaned.columns) - 1)
            ws_corr.freeze_panes(1, 0)

            unmatched_cdpos_cleaned = drop_unwanted_columns(unmatched_cdpos)
            unmatched_cdpos_cleaned.to_excel(writer, sheet_name="Unmatched_CD_Changes", index=False)
            ws_unmatch_cd = writer.sheets["Unmatched_CD_Changes"]
            apply_custom_headers(ws_unmatch_cd, unmatched_cdpos_cleaned, wb)
            ws_unmatch_cd.autofilter(0, 0, len(unmatched_cdpos_cleaned), len(unmatched_cdpos_cleaned.columns) - 1)
            ws_unmatch_cd.freeze_panes(1, 0)

            unmatched_sm20_cleaned = drop_unwanted_columns(unmatched_sm20)
            unmatched_sm20_cleaned.to_excel(writer, sheet_name="Unmatched_SM20_Logs", index=False)
            ws_unmatch_sm20 = writer.sheets["Unmatched_SM20_Logs"]
            apply_custom_headers(ws_unmatch_sm20, unmatched_sm20_cleaned, wb)
            ws_unmatch_sm20.autofilter(0, 0, len(unmatched_sm20_cleaned), len(unmatched_sm20_cleaned.columns) - 1)
            ws_unmatch_sm20.freeze_panes(1, 0)

            # Legend tab remains unchanged
        ...
        log_message(f"Cleaned Excel output successfully generated: {output_file}")
        return True

    except Exception as e:
        log_message(f"Error generating cleaned Excel output: {str(e)}", "ERROR")
        return False
...
def main():
    ...
    try:
        ...
        generate_excel_output_cleaned(correlated_df, unmatched_cdpos, unmatched_sm20, OUTPUT_FILE)
