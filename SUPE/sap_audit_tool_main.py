    ...
    try:
        ...
        if session_df is not None:
            ...
            if MODULAR_MODE:
                ...
                log_message("Using integrated Excel output generation")
                # This would be implemented in the non-modular version
            
        else:
            log_message("No session timeline found. Using legacy correlation mode.")
            ...
            log_message("Applying risk assessment to correlated data...")
            try:
                if MODULAR_MODE:
                    # Use the imported risk assessment function
                    correlated_df = assess_risk_legacy(correlated_df, unmatched_cdpos, unmatched_sm20)
                    # Also assess unmatched SM20 logs
                    if len(unmatched_sm20) > 0:
                        unmatched_sm20['risk_level'] = "Low"
                        unmatched_sm20['risk_factors'] = "Unmatched SM20 log entry"
                else:
                    # Fallback to integrated mode
                    log_message("Using integrated risk assessment")
                    # This would be implemented in the non-modular version
            except Exception as e:
                log_message(f"Error applying risk assessment: {str(e)}", "ERROR")
                # Create empty risk columns to avoid errors later
                correlated_df.loc[:, "risk_level"] = "Unknown"
                correlated_df.loc[:, "risk_factors"] = "Risk assessment failed"
            
            # Step 6: Generate Excel output
            if MODULAR_MODE:
                generate_excel_output(correlated_df, unmatched_cdpos, unmatched_sm20, pd.DataFrame(), OUTPUT_FILE)
            else:
                # Fallback to integrated mode
                log_message("Using integrated Excel output generation")
                # This would be implemented in the non-modular version
